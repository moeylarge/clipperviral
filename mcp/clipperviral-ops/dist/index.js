...
function parseKickStreamers(editorHtmlPath = DEFAULT_EDITOR_HTML) {
    const html = readFileSync(editorHtmlPath, "utf8");
    const start = html.indexOf("const KICK_STREAMERS = [");
    if (start === -1)
        throw new Error(`KICK_STREAMERS array not found in ${editorHtmlPath}`);
    const end = html.indexOf("].map((s)", start);
    if (end === -1)
        throw new Error(`KICK_STREAMERS array end not found in ${editorHtmlPath}`);
...
async function runPythonOverlay(input) {
    ...
    if (input.fontPath)
        args.push("--font-path", input.fontPath);
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            if (code === 0)
                ...
                reject(new Error(stderr || `python3 exited with ${code}`));
        });
    });
}
async function replicateRequest(method, pathname, body) {
    ...
    if (!res.ok)
        throw new Error(`Replicate API ${method} ${pathname} failed: ${res.status} ${raw}`);
    return json;
}
...
async function pollPrediction(id, timeoutMs = 10 * 60_000) {
...
function manualKickProgramStats(period) {
    return {
        period,
...
async function maybeLogSubmission(row) {
    const url = env("SUPABASE_URL");
    const key = env("SUPABASE_SERVICE_ROLE_KEY");
    if (!url || !key)
        return { logged: false, reason: "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not configured." };
    try {
        const supabase = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } });
        const inserted = await supabase.from("clipperviral_submissions").insert(row).select("id").maybeSingle();
        if (inserted.error)
            return { logged: false, reason: inserted.error.message };
...
async function kickApiRequest(method, pathname, body) {
    ...
    const json = raw ? JSON.parse(raw) : {};
    if (!res.ok)
        throw new Error(`Kick Program API ${method} ${pathname} failed: ${res.status} ${raw}`);
...
server.registerTool("clipper_generate_streamer_overlay", {
    ...
    if (!existsSync(DEFAULT_TEMPLATE))
        return errorText(`Missing template: ${DEFAULT_TEMPLATE}`);
...
server.registerTool("clipper_generate_seedance_effect", {
...
server.registerTool("clipper_check_seedance_run", {
...
server.registerTool("clipper_list_missing_streamers", {
...
server.registerTool("clipper_get_program_stats", {
    ...
    if (!api.configured)
        return text(manualKickProgramStats(period));
    return text({ period, ...api.response });
});
server.registerTool("clipper_submit_post_for_payout", {
    ...
    const log = await maybeLogSubmission({
        ...
        submission_id: api.configured ? api.response.id ?? null : null,
...
server.registerTool("clipper_get_viralclips_analytics", {
    ...
    const response = { range, platform, sources: {} };
    if (platform === "x" || platform === "all") {
        ...
        if (!token) {
            ...
            if (!userRes.ok)
                throw new Error(`X API user lookup failed: ${userRes.status} ${userRaw}`);
            ...
            if (!tweetsRes.ok)
                throw new Error(`X API tweets lookup failed: ${tweetsRes.status} ${tweetsRaw}`);
            const tweets = Array.isArray(tweetsJson.data) ? tweetsJson.data : [];
            const topPosts = tweets.map((tweet) => {
                const metrics = (tweet.public_metrics ?? tweet.organic_metrics ?? {});
    ...
    if (platform === "tiktok" || platform === "all") {
        response.sources.tiktok = env("TIKTOK_API_KEY")
